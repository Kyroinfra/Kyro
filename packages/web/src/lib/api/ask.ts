...
export async function semanticSearch(
	apiKey: string,
	q: string,
	opts: { collectionId?: string; fileIds?: string[]; limit?: number; minScore?: number } = {}
): Promise<{ data: SemanticSearchResult[]; query: string; limit: number; minScore: number }> {
...
export async function askStream(
	...
	while (true) {
		...
		for (const rawEvent of events) {
			...
			let parsed: { type: string; sources?: AskSource[]; text?: string; message?: string };
			try {
...
export async function ask(apiKey: string, input: AskInput): Promise<AskNonStreamResponse> {
