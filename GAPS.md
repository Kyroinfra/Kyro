**Gaps that will currently hurt my target users:**

**1. No collection/folder concept**
...
**2. Embedding is fire-and-forget with no visibility**
...
**3. The chunking strategy is naïve for legal/research docs**
...
**5. `/ask` only supports a fixed `fileIds` array**
...
**6. No document metadata/tagging**
...
**Quickest wins for an MVP pivot:**

The `collections` table and scoped `/ask` would unlock all three verticals immediately. The metadata/tagging and reranking can come later but collections + org-wide search mode are table stakes.

Want me to sketch out the schema additions and the route changes for collections + org-wide ask?
