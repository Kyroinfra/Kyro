...
const openapiSpec = {
  ...
  info: {
    title: "Kyro API",
    version: "1.0.0",
    description: "The Kyro API provides programmatic access to file storage, usage metrics, and health monitoring.",
  },
  servers: [
  ...
  tags: [
    { name: "Files", description: "Upload, list, download, and delete files in your Kyro storage." },
    { name: "Usage", description: "Retrieve usage metrics and storage statistics for your organisation." },
    { name: "Health", description: "Check the availability and status of the Kyro backend." },
  ],
  paths: {
    "/files": {
      get: {
        ...
        "x-scope": "read",
        parameters: [
          { name: "limit", in: "query", required: false, description: "Max results to return.", schema: { type: "integer" } },
        ...
        "x-response-example": JSON.stringify({ files: [{ id: "f_abc123", name: "report.pdf", sizeBytes: 204800, createdAt: "2025-01-15T12:00:00Z" }], nextCursor: null }),
      },
      post: {
        summary: "Upload file",
        tags: ["Files"],
        ...
        responses: {
          ...
          "401": { description: "Unauthorized." },
          "413": { description: "File exceeds 100MB limit." },
        },
        "x-response-example": JSON.stringify({ id: "f_abc123", name: "report.pdf", sizeBytes: 204800, createdAt: "2025-01-15T12:00:00Z" }),
      },
    },
    "/files/{id}": {
      get: {
        ...
        "x-scope": "read",
        parameters: [
          { name: "id", in: "path", required: true, description: "File UUID.", schema: { type: "string", format: "uuid" } },
        ],
        responses: {
    ...
    "/usage": {
      get: {
        ...
        "x-response-example": JSON.stringify({ totalFiles: 142, totalSizeBytes: 1073741824, uploadsThisMonth: 38 }),
      },
    },
    "/health": {
      get: {
        ...
        "x-response-example": JSON.stringify({ status: "ok", database: "connected", uptime: 86400 }),
      },
    },
  },
};
    ...
    const TAG_ORDER = ["Files", "Usage", "Health"];
                ...
                {#each allTags as tag}
                    <div class="sidebar-group">
                        ...
                        {#each tag.endpoints as ep}
                            ...
                            >
                                <span
                                    class="sidebar-method method-{ep.method.toLowerCase()}"
                                    >{ep.method}</span
                                >
        ...
        <main class="content">
            ...
            <section id="sdk" class="doc-section">
                ...
                <p class="section-desc">
                    The official SDK is the recommended way to integrate Kyro.
                    Install it, pass your API key from the dashboard, and start
                    uploading files immediately.
                </p>
                ...
                <div class="info-grid" style="margin-bottom: 20px">
                    ...
                    <div class="info-card">
                        <span class="info-label">Auth</span>
                        <code class="info-value">API Key (from dashboard)</code>
                    </div>
                    <div class="info-card">
                ...
                <div class="steps">
                    <div class="step">
                        ...
                        <div class="step-body">
                            <h4>Create an account & get your API key</h4>
                            <p class="section-desc" style="margin-bottom: 10px">
                                Deploy your instance, then navigate to
                                <strong>Dashboard → API Keys</strong> and create
                                a key with the scopes you need.
                            </p>
                        ...
                        <div class="code-block">
                            <div class="code-toolbar">
                                ...
                                <button
                                    ...
                                    onclick={() =>
                                        copy(
                                            `const uploaded = await kyro.files.uploadFile(file, "report.pdf");\nconsole.log(uploaded.id, uploaded.sizeBytes);`,
                                            "sdk-upload",
                                        )}
                                >
                        ...
                        <div class="code-block">
                            <div class="code-toolbar">
                                ...
                                <button
                                    ...
                                    onclick={() =>
                                        copy(
                                            `const files = await kyro.files.listFiles(50);\nfor (const f of files) {\n  console.log(f.name, f.sizeBytes);\n}`,
                                            "sdk-list",
                                        )}
                                >
                        ...
                        <div class="code-block">
                            <div class="code-toolbar">
                                ...
                                <button
                                    ...
                                    onclick={() =>
                                        copy(
                                            `const blob = await kyro.files.downloadFile("file-id");\nconst url = URL.createObjectURL(blob);`,
                                            "sdk-download",
                                        )}
                                >
...
<span class="ck">const</span> url = URL.<span class="cs">createObjectURL</span>(blob);</code></pre>
                        ...
                        <div class="code-block">
                            <div class="code-toolbar">
                                ...
                                <button
                                    ...
                                    onclick={() =>
                                        copy(
                                            `await kyro.files.deleteFile("file-id");`,
                                            "sdk-delete",
                                        )}
                                >
            ...
            <section id="sdk-health" class="doc-section">
                ...
                <p class="section-desc">
                    Check whether the Kyro backend is reachable. Useful for
...
console.log(status.status); <span class="c-comment">// "ok" | "degraded"</span></code></pre>
            ...
            <section id="rate-limiting" class="doc-section">
                ...
                <p class="section-desc">
                    All endpoints are rate-limited per API key. Exceeding the
                    limit returns <code>429 Too Many Requests</code>.
                </p>
...
<style>
    ...
    *::after {
    ...
    :root {
        ...
        /* Spacing tokens so breakpoints are consistent */
        --content-pad-lg: 56px;
        --content-pad-md: 32px;
        --content-pad-sm: 20px;
        --content-pad-xs: 16px;
    ...
    .topbar {
        ...
        /* Prevent content from ever running under */
        flex-shrink: 0;
    }
    ...
    /* Hamburger: always visible — sidebar starts hidden on all screens,
       only truly fixed-open on large screens via the layout below */
    .hamburger {
        display: flex;
        align-items: center;
    ...
    .layout {
        ...
        max-width: 1440px;
        margin: 0 auto;
        /* KEY FIX: don't lock layout to viewport height with overflow:hidden.
           Let each column scroll naturally. The sidebar gets its own scroll. */
    ...
    ───────────────────────────────────────── */
    .sidebar {
        /* On large screens, sidebar lives in flow */
        width: 224px;
        flex-shrink: 0;
        /* Sticky within the layout column */
        position: sticky;
        top: 50px;
        ...
        /* Start hidden on all sizes; .open toggles it */
        transform: translateX(-100%);
        /* On desktop (≥900px) we override this below */
        position: fixed;
        top: 50px;
        left: 0;
        height: calc(100vh - 50px);
    ...
    /* Close row — always rendered, only shown when sidebar is a drawer */
    .sidebar-close-row {
        display: flex;
        align-items: center;
        justify-content: space-between;
    ...
    .content {
        ...
        min-width: 0;
        padding-bottom: 80px;
        /* No fixed height / overflow-hidden: natural document flow */
    }
    ...
    .info-grid {
        display: grid;
        /* Fluid columns: min 160px so they collapse gracefully */
    ...
    /* ─────────────────────────────────────────
       TABLE SCROLL WRAPPER — prevents tables
       from breaking layout on narrow screens
    ───────────────────────────────────────── */
    .table-scroll {
        width: 100%;
        overflow-x: auto;
    ...
    .param-table {
        width: 100%;
        min-width: 400px; /* force scroll rather than wrapping weirdly */
    ...
    .ep-left {
        ...
        flex-direction: column;
        gap: 24px;
        /* Ensure it doesn't shrink to zero on edge cases */
        min-width: 0;
    }
    ...
    .ep-right {
        ...
        gap: 14px;
        min-width: 0;
        /* Sticky code panel on large screens */
        position: sticky;
        top: 50px;
    ...
    .auth-bearer  { background: var(--blue-dim);  color: var(--blue);       border-color: rgba(91, 156, 246, 0.2);  }
    .auth-apikey  { background: var(--green-dim); color: var(--green);      border-color: rgba(61, 214, 140, 0.2); }
    .auth-none    { background: var(--bg3);        color: var(--text-ghost); border-color: var(--border);            }
    ...
    .ck       { color: var(--green); }
    .cs       { color: var(--amber); }
    .c-comment{ color: var(--text-ghost); font-style: italic; }

    /* ═══════════════════════════════════════════
       RESPONSIVE — working from largest to smallest
    ═══════════════════════════════════════════ */

    /* ── ≥900px: sidebar becomes a persistent panel in the document flow ── */
    @media (min-width: 900px) {
        .sidebar {
            /* Un-fix the sidebar; put it back into flow */
            position: sticky;
            top: 50px;
            height: calc(100vh - 50px);
            /* Always visible — never translated away */
            transform: none !important;
            z-index: auto;
        }

        /* Hide the drawer controls since sidebar is always open */
        .sidebar-close-row { display: none; }
        /* Hide the hamburger (sidebar is always visible) */
        .hamburger { display: none; }
    }

    /* ── 1100px: collapse endpoint two-column to stacked ── */
    ...
    @media (max-width: 640px) {
        ...
        /* Hide "API Reference" text label on very small screens */
        .topbar-label {
            display: none;
        }
        ...
        /* Stack step number and body vertically on tiny screens */
        .step {
            flex-direction: column;
            gap: 10px;
        }
